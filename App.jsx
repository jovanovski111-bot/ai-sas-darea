    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        <div className="modal-header">
          <div className="modal-title">📋 {filename}</div>
          <button className="btn btn-ghost btn-icon" onClick={onClose}>✕</button>
        </div>
        <div className="modal-body">
          <div style={{fontSize:12,color:"var(--text3)",marginBottom:10}}>
            Copy the CSV data below and paste it into a text editor or spreadsheet app.
          </div>
          <textarea
            readOnly
            value={csv}
            ...
            style={{
              width:"100%", height:220, background:"var(--bg3)", border:"1px solid var(--border2)",
              borderRadius:"var(--radius2)", padding:"10px 12px", fontFamily:"var(--mono)",
              fontSize:11, color:"var(--text2)", resize:"vertical", outline:"none", lineHeight:1.6,
            }}
          />
        </div>
...
function CRMPage({ toast }) {
  ...
  const exportCSV = () => {
    ...
    const csv = rows.map(r=>r.join(",")).join("\n");
    setCsvModal({ csv, filename: "leads.csv" });
  };
    ...
    <div>
      ...
      {confirmDelete && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setConfirmDelete(null)}>
          <div className="modal">
            <div className="modal-header"><div className="modal-title">Delete Event</div><button className="btn btn-ghost btn-icon" onClick={()=>setConfirmDelete(null)}>✕</button></div>
            <div className="modal-body"><p style={{fontSize:13,color:"var(--text2)"}}>Delete "<strong>{confirmDelete.title}</strong>"?</p></div>
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={()=>setConfirmDelete(null)}>Cancel</button>
              <button className="btn btn-danger" onClick={()=>{setEvents(p=>p.filter(x=>x.id!==confirmDelete.id));toast("Event deleted");setConfirmDelete(null);}}>Delete</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="chat-messages">
              ...
              {active?.messages.map((m, i) => (
                <div key={i} className={`chat-msg ${m.role}`}>
                  ...
                  <div className="chat-bubble">{renderMsg(m.content)}</div>
                </div>
              ))}
              {typing && (
